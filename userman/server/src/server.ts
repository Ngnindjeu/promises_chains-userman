...
io.sockets.on("connection", (socket) => {
    ...
    socket.on('connect', () => {
        socket.emit('state', clientStates)
    })
    ...
    socket.on('edit', ({user}) => {
        clientStates.lockedUsers[socket.id] = user.id
        io.emit('state', clientStates)
        io.emit('lock', user.id)
    })

    socket.on('close', ({user}) => {
        delete clientStates.lockedUsers[socket.id]
        io.emit('state', clientStates)
        io.emit('unlock', user.id)
    })

    socket.on('requestUpdate', () => {
        socket.broadcast.emit('getList')
    })
...
function isPrivilegedAtLeast(rights: Rights) {
    ...
    return (req: Request, res: Response, next) => {
        if (rights > Number((req.session as any).user.rights)) {
...
app.get('/login', isLoggedIn(), (req: Request, res: Response) => {
    res.status(200).send({
        message: 'User still logged in',
        user: (req.session as any).user, // Send user object to client for greeting message
    });
});
 ...
 * }
 */
app.post('/login', async (req: Request, res: Response) => {     // wird async
...
app.post('/logout', (req: Request, res: Response) => {
    // Log out user
    delete (req.session as any).user; // Delete user from session
    res.status(200).send({
        message: 'Successfully logged out',
    });
});
